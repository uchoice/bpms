...
public class BaseProcessDefinitionDiagramLayoutResource {
  ...
  // objectMapper提成公用的  update by xbyang 2017年7月3日21:18:38
  ...
  public ObjectNode getDiagramNode(String processInstanceId, String processDefinitionId) {
    ...
    if (processDefinition.getParticipantProcess() != null) {
      ...
      ObjectNode participantProcessJSON = objectMapper.createObjectNode();
  ...
  private void getActivity(String processInstanceId, ActivityImpl activity, ArrayNode activityArray, 
    ...
    // update by xbyang 2017年7月3日10:48:16
    if(activityBehavior instanceof UserTaskActivityBehavior){
    	String assign = "";
    	ArrayNode candidateGroup = objectMapper.createArrayNode();
    	ArrayNode candidateUserId = objectMapper.createArrayNode();
    	TaskDefinition taskDefinition = ((UserTaskActivityBehavior) activityBehavior).getTaskDefinition();
    	Expression assignExp = taskDefinition.getAssigneeExpression();
    	Set<Expression> candidateGroupExp = taskDefinition.getCandidateGroupIdExpressions();
    	Set<Expression> candidateUserIdExp = taskDefinition.getCandidateUserIdExpressions();
    	if(assignExp != null){
    		assign = assignExp.getExpressionText();
    	}
    	if(candidateGroup != null){
    		for(Expression exp : candidateGroupExp){
    			candidateGroup.add(exp.getExpressionText());
    		}
    	}
    	if(candidateUserIdExp != null){
    		for(Expression exp : candidateUserIdExp){
    			candidateUserId.add(exp.getExpressionText());
    		}
    	}
    	propertiesJSON.put("assignExp", assign);
    	propertiesJSON.put("candidateGroupExp", candidateGroup);
    	propertiesJSON.put("candidateUserExp", candidateUserId);
    }
